...
import { ListChecks, ClipboardList, HandCoins, Banknote, AlertTriangle, Wallet, TrendingUp, Users } from "lucide-react";
...
export const Route = createFileRoute("/_app/usp/")({
  head: () => ({ meta: [{ title: "Dashboard USP · BUMDes" }] }),
  component: UspDashboardPage,
});
...
function UspDashboardPage() {
  ...
  const outstanding = dr(acc(USP_ACCOUNT_CODES.piutang)?.id);
  const kasUsp = dr(acc(USP_ACCOUNT_CODES.kas)?.id) + dr(acc(USP_ACCOUNT_CODES.bank)?.id);
  const bungaBulan = cr(acc(USP_ACCOUNT_CODES.bunga)?.id, balMonth);
  const dendaBulan = cr(acc(USP_ACCOUNT_CODES.denda)?.id, balMonth);
  const bebanBulan = dr(acc(USP_ACCOUNT_CODES.beban)?.id, balMonth);
  ...
  return (
    <>
      <PageHeader
        title="Dashboard USP"
        subtitle="Ringkasan kinerja Unit Simpan Pinjam — pinjaman, kas, dan laba berjalan."
      />
        ...
        <div className="glass-card rounded-2xl p-4 sm:p-5">
          ...
          {!recent || recent.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Belum ada aktivitas USP. <Link to="/usp/kegiatan" className="text-[var(--neon-cyan)] hover:underline">Catat kegiatan</Link>.
            </p>
          ) : (
